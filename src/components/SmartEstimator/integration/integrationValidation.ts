...
export function validateEstimatorIntegration(): IntegrationValidationResult {
  ...
  const checks: IntegrationValidationCheck[] = [
    ...
    { name: 'Residential appliance repair displays a diagnostic range and parts allowance', passed: residentialAppliance.estimate.resultHeading === 'Preliminary Diagnostic / Initial Service Range' && residentialAppliance.estimate.applicableLaborRate === COMPANY_STANDARDS.laborRates.residential && residentialAppliance.estimate.materials !== '$0' },
    { name: 'Commercial appliance repair displays a commercial diagnostic range', passed: commercialAppliance.estimate.resultHeading === 'Preliminary Diagnostic / Initial Service Range' && commercialAppliance.estimate.applicableLaborRate === COMPANY_STANDARDS.laborRates.commercial && hasDollarRange(commercialAppliance.estimate) },
    ...
    { name: 'Unspecified plumbing work displays a preliminary plumbing service range', passed: plumbingWork.estimate.resultHeading === 'Preliminary Diagnostic / Initial Service Range' && hasDollarRange(plumbingWork.estimate) },
