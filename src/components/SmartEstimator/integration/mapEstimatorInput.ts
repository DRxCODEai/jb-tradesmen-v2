...
export function mapEstimatorInput(data: Data, serviceId: string): MappedEstimatorInput {
  ...
  const answers: Record<string, EstimatorQuestionValue> = {
    ...
    activeArcing: phrase(combined, /\b(active arcing|active sparking|sparking now)\b/),
    electricalArcing: phrase(combined, /\b(active arcing|active sparking|sparking now)\b/),
