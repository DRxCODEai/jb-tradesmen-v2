...
export function useSmartEstimator() {
  ...
  const reset = () => { data.photos.forEach((photo) => URL.revokeObjectURL(photo.url)); photosRef.current = []; estimateRef.current = null; setData(initial); setEstimate(null); setStep(0); setError(''); setInvalidQuestionId(null); window.requestAnimationFrame(() => document.querySelector<HTMLElement>('.smart-step h2')?.focus({ preventScroll: true })) }
